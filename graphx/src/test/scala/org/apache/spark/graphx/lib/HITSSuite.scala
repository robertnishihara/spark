...
class HITSSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("Chain HITS") {
    withSpark { sc =>
      ...
      val verticesGroundTruth1: RDD[(VertexId, (Double, Double))] = sc.parallelize(
        Array((1L, (0.0, 1.0 / 4.0)), (2L, (1.0 / 4.0, 1.0 / 4.0)), (3L, (1.0 / 4.0, 1.0 / 4.0)),
          (4L, (1.0 / 4.0, 1.0 / 4.0)), (5L, (1.0 / 4.0, 0.0)))
      )
  ...
  test("Ring HITS") {
    withSpark { sc =>
      ...
      val edges: RDD[Edge[Unit]] = sc.parallelize(
        Array(Edge(1L, 2L, ()), Edge(2L, 3L, ()), Edge(3L, 4L, ()), Edge(4L, 1L, ()))
      )
      val graph = Graph(vertices, edges)
      ...
      val rankGraph1 = HITS.run(graph, 1)
      val verticesGroundTruth1: RDD[(VertexId, (Double, Double))] = sc.parallelize(
        Array((1L, (1.0 / 4.0, 1.0 / 4.0)), (2L, (1.0 / 4.0, 1.0 / 4.0)), (3L, (1.0 / 4.0, 1.0 / 4.0)),
          (4L, (1.0 / 4.0, 1.0 / 4.0)))
      )
      assert(compareScores(rankGraph1.vertices, VertexRDD(verticesGroundTruth1)) < 1e-6)
